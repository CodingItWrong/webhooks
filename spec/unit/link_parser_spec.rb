...
RSpec.describe LinkParser, :vcr do
    ...
    context "when there are redirects" do
      ...
      it "follows redirects" do
        expect(link.canonical).to eq(
          "https://bignerdranch.com/blog/testing-external-dependencies-with-fakes/"
        )
      end
    end

    context "when the request errors" do
      let(:url) { "https://screenrant.com/malcolm-x-movie-true-story-accuracy-changes-missing-details/" }
  ...
  describe "#title" do
    ...
    context "when there just isn't a title tag at all" do
      ...
      it "uses the last path segment as the title, titleized" do
